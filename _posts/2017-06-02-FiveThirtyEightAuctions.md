...
This is a bit ugly, let's see if we can exploit our symmetry a bit more. Remember that $$B(\theta)=b$$ so $$B^{-1}(b)=\theta)$$. So the foc is actually $$G(\theta)B'(\theta)+g(\theta)B(\theta)=\theta g(\theta)$$. But this should look pretty familiar, its just $$\frac{d}{d\theta}(G(\theta)B(\theta))=\theta g(\theta) $$ Let's remember we just want to solve for $$ B(\theta)$$ but that's no problem now. So algebra yada-yada and we get $$B(\theta)=\frac{1}{G(\theta)}\int_0^\theta y g(y) dy$$. Which is just the exepectation of $$y_1$$ conditional of it being less than $$\theta$$ (or in words, the expected value of second highest type conditional on player 1's type being the highest).
